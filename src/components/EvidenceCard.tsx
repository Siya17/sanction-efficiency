...
import type { EvidenceSortCategory } from "../types";
import { categoryLabels, evidenceTypeLabels } from "../utils/labels";

type EvidenceCardProps = {
  card: any; // We can use any here to accept both EvidenceCard and StudentEvidenceCard
...
export function EvidenceCard({ card, category, onAssign, onDelete }: EvidenceCardProps) {
  const isStudentAdded = card.isStudentAdded || ('summary' in card);
  const text = card.text || card.summary;
  const whyItMatters = card.whyItMatters || card.explanation;
    ...
    <article className={`evidence-card ${category || ''} ${isStudentAdded ? "border-2 border-indigo-400" : ""}`}>
      <div className="flex justify-between items-start mb-2">
        <div className="flex flex-wrap gap-2">
          ...
          {!isStudentAdded && card.type && <span className="bg-blue-50 text-blue-700 text-xs px-2 py-1 rounded">{evidenceTypeLabels[card.type as keyof typeof evidenceTypeLabels]}</span>}
      ...
      {whyItMatters && (
        <div className="mb-2 bg-blue-50 border border-blue-100 p-2 rounded text-xs text-blue-800">
          <strong className="block mb-1">Why it matters:</strong> {whyItMatters}
        </div>
      )}
